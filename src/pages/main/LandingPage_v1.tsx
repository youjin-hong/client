...
const LandingPage = () => {
  return (
    <>
      <div className="relative w-full h-full min-h-screen flex flex-col max-lg:min-h-fit">
        {/* 애니메이션 배경을 lazy loading으로 처리 */}
        <Suspense fallback={<div className="absolute inset-0 bg-gradient-to-br from-gray-50 to-gray-100" />}>
          {/* <AnimatedBackground /> */}
          <CheckeredBackground />
        </Suspense>
        <div className="z-10 grid grid-cols-2 w-full h-screen items-center justify-center px-[8vw] max-lg:grid-cols-1 max-lg:place-items-center max-lg:h-fit max-lg:min-h-screen">
          <div className="flex flex-col justify-center flex-1 max-w-[600px] p-12 rounded-[50px] bg-[#F0FEED]/50">
            <img
              src={bigLogo}
              alt="AUTA 로고"
              className="w-48 mb-8 max-lg:w-36"
              loading="eager" // 로고는 즉시 로드
            />
            <h1 className="text-4xl font-extrabold mb-6 max-lg:text-xl">AUTA No-Code 테스트</h1>
            <div className="text-16 mb-8 leading-relaxed font-semibold max-lg:text-[12px]">
              🎯 디자인 그대로, 자동으로 테스트하다
              <br />
              <br />
